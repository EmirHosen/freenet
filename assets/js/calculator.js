...
//variables
let NPr = 1, P = 10000, S = 20000, CD = 0, Sum = 0, PI = 1, MI = 1, Total = 0;
...
//get result
function Calculate() {
  Total = (((NPr * 10000 + P) * PI) + S + Sum) * MI;
  
  if (Total > 100) {
    CD = 0;
    domain.attr('data-value', 'رایگان');
    $('#domain_label').text('رایگان');
  } else {
    domain.attr('data-value', '+ 10،000تومان');
    $('#domain_label').text('10،000تومان');
  }
  
  Total = (((NPr * 10000 + P) * PI) + S + Sum + CD) * MI;
...
function Bill() {
  ...
  $('#projects-price').text(NPr * 10000 + 'تومان');
  ...
  //Total
  $('.calculator__sidebar-price-total').text(Total + ' تومان');
